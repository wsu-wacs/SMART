...
function Duplicates=CheckDuplicatesUserAgent(Similar,UserAgent,Duplicates)
    [FileNew,Order]=sort(UserAgent(Similar));
    CheckF=1;
        ...
        for InerCheckF=CheckF+1:numel(FileNew)
            if(strcmp(FileNew(CheckF),FileNew(InerCheckF)))
                ...
                if(InerCheckF==numel(FileNew))
                    % Find at least two requests all fields are identical
                    Duplicates=[Duplicates; SimilarF(1:end-1)];
                end
