...
function Duplicates=CheckDuplicatesIP(Similar,IP,UserAgent,Duplicates)
    [FileNew,Order]=sort(IP(Similar));
    CheckF=1;
        ...
        for InerCheckF=CheckF+1:numel(FileNew)
            if(strcmp(FileNew(CheckF),FileNew(InerCheckF)))
                ...
                if(InerCheckF==numel(FileNew))
                    % Find at least two requests ... and IP fields are identical
                    Duplicates=CheckDuplicatesUserAgent(SimilarF,UserAgent,Duplicates);
                end
