...
function Duplicates=CheckDuplicatesErrorCode(Similar,ErrorCode,Referrer,DataVolume,IP,UserAgent,Duplicates)
    [FileNew,Order]=sort(ErrorCode(Similar));
    CheckF=1;
        ...
        for InerCheckF=CheckF+1:numel(FileNew)
            if(strcmp(FileNew(CheckF),FileNew(InerCheckF)))
                ...
                if(InerCheckF==numel(FileNew))
                    %Found more than two duplicates in ... and ErrorCode->go to next check
                    Duplicates=CheckDuplicatesReferrer(SimilarF,Referrer,DataVolume,IP,UserAgent,Duplicates);
                end
