...
function Duplicates=CheckDuplicatesDataVolume(Similar,DataVolume,IP,UserAgent,Duplicates)
    [FileNew,Order]=sort(DataVolume(Similar));
    CheckF=1;
        ...
        for InerCheckF=CheckF+1:numel(FileNew)
            if(strcmp(FileNew(CheckF),FileNew(InerCheckF)))
                ...
                if(InerCheckF==numel(FileNew))
                    %Found more than two duplicates in ... and DataVolum->go to next check
                    Duplicates=CheckDuplicatesIP(SimilarF,IP,UserAgent,Duplicates);
                end
